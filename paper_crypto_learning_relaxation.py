...
def _unbounded_learning() -> bool:
    """Permit unlimited cadence only inside the explicitly disarmed paper sandbox."""
...
def install_paper_crypto_learning_relaxation() -> bool:
    ...
    simulated cash/reserve/liquidity constraints, accounting, execution claims,
    and every live-order control remain authoritative.
    """
    global _INSTALLED
    if _INSTALLED:
        return True
    ...
    def paper_pre_trade_risk_checks(**kwargs):
        ...
        if _unbounded_learning():
            # Keep execution/data-integrity checks but make model/risk throttles
            # observational rather than vetoes inside the paper sandbox.
