...
def install_paper_optimizer_size_handoff() -> bool:
    ...
    Stock penny-policy globals are deliberately left untouched. Low-priced crypto
    sizing is handled locally by the optimizer-approved paper allocation path so
    importing/installing this crypto wrapper cannot weaken stock safeguards or
    contaminate stock policy tests in the same Python process.
    """
    global _INSTALLED
    if _INSTALLED:
        return True
