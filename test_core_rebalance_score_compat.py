...
def test_candidate_keeps_confidence_and_explicit_hold_requirements():
    ...
    try:
        ...
        eligible = SimpleNamespace(action="HOLD", score=0.5376, confidence=0.5526)
        low_confidence = SimpleNamespace(action="HOLD", score=0.90, confidence=0.47)
        non_hold = SimpleNamespace(action="SELL", score=0.90, confidence=0.90)
...
def test_no_deployment_gap_still_blocks_candidate():
    ...
    try:
        compat.install_core_rebalance_score_compat()
        signal = SimpleNamespace(action="HOLD", score=0.90, confidence=0.90)
        assert patch._core_rebalance_candidate_allowed(
            signal,
            market="crypto",
            deployment_gap=0.0,
