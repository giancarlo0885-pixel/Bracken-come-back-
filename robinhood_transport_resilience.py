...
def _pace_marketdata(path: str) -> None:
    ...
    if "/api/v2/crypto/marketdata/" not in str(path or ""):
        return
    # Robinhood market-data reads are shared by several concurrent worker paths.
    # Serialize only the authenticated market-data request itself and keep a small
    # interval between reads so fast/deep scans do not create an avoidable burst.
...
def install_robinhood_transport_resilience() -> bool:
    ...
    HTTP status information without response bodies or credentials.
    """
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                if not retryable or attempt >= attempts - 1:
                    log.warning(
                        "ROBINHOOD GET FAIL | endpoint=%s | status=%s | attempts=%d | error=%s",
                ...
                log.info(
                    "ROBINHOOD GET RETRY | endpoint=%s | status=%s | attempt=%d/%d | delay=%.2fs",
                    str(path or "").split("?", 1)[0],
                    status if status is not None else "transport",
                    attempt + 1,
                    attempts,
    ...
    log.info(
        "ROBINHOOD TRANSPORT RESILIENCE | get_retry=ON | marketdata_pacing=0.20s | live_order_submission=UNCHANGED"
    )
    return True
