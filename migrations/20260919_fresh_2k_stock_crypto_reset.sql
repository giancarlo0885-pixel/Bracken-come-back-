-- Fresh $2,000 paper reset for both stock ("cash") and crypto portfolios.
-- Archive active portfolio/accounting state first so the failed run remains
-- available for audit while the active books restart cleanly.
...
SELECT 'fresh-2k-20260919', 'portfolios', to_jsonb(row_data), timezone('utc', now())::text
FROM portfolios AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'positions', to_jsonb(row_data), timezone('utc', now())::text
FROM positions AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'trades', to_jsonb(row_data), timezone('utc', now())::text
FROM trades AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'trade_ledger', to_jsonb(row_data), timezone('utc', now())::text
FROM trade_ledger AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'position_lots', to_jsonb(row_data), timezone('utc', now())::text
FROM position_lots AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'executions', to_jsonb(row_data), timezone('utc', now())::text
FROM executions AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'equity_snapshots', to_jsonb(row_data), timezone('utc', now())::text
FROM equity_snapshots AS row_data
WHERE market IN ('cash', 'crypto');

INSERT INTO paper_reset_archive(reset_id, source_table, row_data, archived_at)
SELECT 'fresh-2k-20260919', 'portfolio_rotations', to_jsonb(row_data), timezone('utc', now())::text
