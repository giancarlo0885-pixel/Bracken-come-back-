...
def _retention_selector(table: str) -> tuple[str, str, str]:
    """Return the key/order/predicate for rows eligible for rolling retention."""
...
def _retention_cleanup_due(conn: Any, table: str, *, keep_rows: int, batch_size: int) -> bool:
    ...
    For global_decision_ledger, canonical provenance linked to a trade, execution
    claim, or realized forecast outcome is excluded from retention entirely.
    """
    slack_rows = max(batch_size, max(1, keep_rows // 5))
    key_column, order_column, eligible_sql = _retention_selector(table)
    probe = conn.execute(
        f"SELECT {key_column} FROM {table} WHERE {eligible_sql} "
        f"ORDER BY {order_column} DESC OFFSET %s LIMIT 1",
