...
def provider_health_summary(*, maximum_age_minutes: int = 90) -> dict[str, Any]:
    """Report health only from independent confirmed market quote evidence.

    ``quote_verifications`` represents external market-data agreement. Paper fills
    are intentionally excluded because they are generated by Oracle's own execution
    path and therefore cannot independently prove provider health. Broker/paper-fill
    evidence may be surfaced elsewhere for execution diagnostics, but it must not
    satisfy this hard capital-readiness gate.
    """
