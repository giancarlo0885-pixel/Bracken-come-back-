...
def evidence_summary() -> dict[str, Any]:
    with connect() as conn:
        counts = conn.execute(
            ...
            SELECT
              ...
              COUNT(*) FILTER (WHERE status='ABSTAIN_PENDING') AS abstentions_pending,
              COUNT(*) FILTER (WHERE status='ABSTAIN') AS legacy_abstentions_unresolved,
    ...
    return {
        ...
        "legacy_abstentions_unresolved": int(counts.get("legacy_abstentions_unresolved") or 0),
...
def governance_summary() -> dict[str, Any]:
    ...
    # promote itself merely by accumulating predictions or abstentions.
